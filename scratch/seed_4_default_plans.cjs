...
const DEFAULT_PLANS = [
  {
    ...
    features: [
      ...
      `__SPECS__:${JSON.stringify({
        description: 'Ideal for small cafes & food stalls starting with digital ordering',
...
async function seedDefaultPlans() {
  console.log('=== SEEDING 4 DEFAULT SaaS PLANS INTO SUPABASE ===\n');
  ...
  for (const p of DEFAULT_PLANS) {
    ...
    if (error) {
      ...
      console.log(`✅ Successfully seeded plan: ${p.name} (₹${p.price_monthly}/mo)`);
    }
  }
