...
async function runPhase10Audit() {
  console.log('==================================================');
  console.log('=== PHASE-10 LIVE PRODUCTION ORDERING AUDIT    ===');
  console.log('==================================================');

  const browser = await chromium.launch({ headless: true });

  // 1. Customer Scans Table 1 QR & Places Real Order with Variants & Cooking Notes
  console.log('\n[1] CUSTOMER QR ORDERING (TABLE 1)...');
  const customerContext = await browser.newContext({ viewport: { width: 412, height: 915 } }); // Mobile viewport
  ...
  await customerPage.screenshot({ path: cartScr, fullPage: true });
  ...
  await customerPage.screenshot({ path: trackingScr, fullPage: true });
  ...
  // 2. Kitchen Display System (KDS) Receives Order
  ...
  if (acceptBtn) {
    ...
    await kdsPage.waitForTimeout(2000);
    console.log('KDS accepted ticket -> PREPARING');
  }

  // 3. Waiter Portal Receives Order
  console.log('\n[3] WAITER PORTAL...');
  ...
  await waiterPage.screenshot({ path: waiterScr, fullPage: true });
  ...
  // 4. Cashier Settles Bill
  console.log('\n[4] CASHIER BILLING & SETTLEMENT...');
  ...
  await cashierPage.screenshot({ path: cashierScr, fullPage: true });
  ...
  // Settle Bill
  const settleBtn = await cashierPage.$('button:has-text("Settle"), button:has-text("Accept Order"), button:has-text("Paid")');
  if (settleBtn) {
    await settleBtn.click();
    await cashierPage.waitForTimeout(2000);
    console.log('Cashier processed settlement.');
  }

  // 5. Owner Reports Reflect Transaction
  console.log('\n[5] OWNER REPORTS...');
  ...
  await ownerPage.screenshot({ path: reportsScr, fullPage: true });
  ...
  // Database verification of created order
