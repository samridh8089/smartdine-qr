...
async function runLiveVerificationNow() {
  console.log('====================================================');
  console.log('   FIREBASE ADMIN FCM V1 LIVE VERIFICATION PIPELINE  ');
  console.log('====================================================\n');

  // Detect connected ADB device
  let deviceSerial = 'RZGYB1BYC4J';
  try {
    const devicesOutput = execSync('adb devices').toString();
    const lines = devicesOutput.split('\n').filter(l => l.includes('\tdevice'));
    if (lines.length > 0) {
      deviceSerial = lines[0].split('\t')[0].trim();
    }
  } catch (e) {}

  console.log(`Detected Connected ADB Device Serial: ${deviceSerial}`);

  // 1. Install & Launch App
  console.log(`Step 1: Installing fresh Release APK onto device ${deviceSerial}...`);
  const apkPath = 'smartdine-mobile/android/app/build/outputs/apk/release/app-release.apk';
  try {
    execSync(`adb -s ${deviceSerial} install -r "${apkPath}"`);
    execSync(`adb -s ${deviceSerial} shell pm grant com.smartdine.mobile android.permission.POST_NOTIFICATIONS`);
    execSync(`adb -s ${deviceSerial} shell monkey -p com.smartdine.mobile 1`);
  } catch (e) {
    console.log('APK Install / Launch Warning:', e.message);
  }

  console.log('\nStep 2: Waiting 5 seconds for app startup auto-sync to execute...');
  await new Promise(r => setTimeout(r, 5000));

  // 2. Read Auto-Synced DB Profile Token for logged-in user nakshatra1233@gmail.com
  console.log('Step 3: Reading DB Profile Record directly from Supabase for nakshatra1233@gmail.com...');
  let { data: dbProfile } = await client
    .from('profiles')
    .select('id, email, role, push_token, updated_at')
    .eq('id', '6f0bf0d3-f87e-4583-861c-262fb44720af')
    .maybeSingle();

  if (!dbProfile) {
    const { data: allP } = await client.from('profiles').select('id, email, role, push_token, updated_at').order('updated_at', { ascending: false });
    dbProfile = allP?.[0];
  }

  console.log('Live App Auto-Synced DB Profile Record:\n', JSON.stringify(dbProfile, null, 2));

  const targetToken = dbProfile?.push_token;
  console.log(`Target FCM Registration Token: ${targetToken}`);

  // 3. Put app in Background state (Swipe-Away)
  console.log('\nStep 4: Pressing Home button to put app in Background/Swipe-Away state...');
  try {
    execSync(`adb -s ${deviceSerial} shell input keyevent 3`);
  } catch (e) {}

  await new Promise(r => setTimeout(r, 1000));
  try { execSync(`adb -s ${deviceSerial} logcat -c`); } catch (e) {}

  // 4. Start 15-second MP4 continuous screen recording
  console.log('Step 5: Starting 15-second MP4 continuous screen recording on physical device...');
  const screenRec = spawn('adb', ['-s', deviceSerial, 'shell', 'screenrecord', '--time-limit=15', '/data/local/tmp/fcm_v1_live_proof.mp4']);
  await new Promise(r => setTimeout(r, 1500));

  const nowTime = new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit' });

  // 5. Create Customer Call DB Entry
  ...
  console.log(`\nStep 6: [DB SUCCESS] Created Customer Call Request ID: ${call?.[0]?.id} for Table 1`);

  // 6. Dispatch Push Notification via Firebase Admin FCM v1
  ...
  try {
    ...
    console.log(`\n[FIREBASE ADMIN FCM V1 SUCCESS] Latency: ${latency}ms | Message ID: ${fcmMessageId}`);
  } catch (fcmErr) {
    console.log('\n[FIREBASE ADMIN FCM V1 ERROR]:', fcmErr.message);
  }

  // 7. Capture ADB Logcat System Trace
  await new Promise(r => setTimeout(r, 2000));
  console.log('\nStep 8: Capturing ADB Logcat system trace for FirebaseMessaging & NotificationManager...');
  try {
    const rawLogcat = execSync(`adb -s ${deviceSerial} logcat -d`).toString();
  ...
  // 8. Pull Screen Recording
  console.log('\nWaiting for MP4 screen recording to finish...');
  await new Promise(r => setTimeout(r, 9000));

  try {
    execSync(`adb -s ${deviceSerial} pull /data/local/tmp/fcm_v1_live_proof.mp4 fcm_v1_live_proof.mp4`);
