...
async function testExpoPushKilled() {
  console.log('====================================================');
  console.log('   EXPO PUSH KILLED STATE LIVE BELL ALERT TEST      ');
  console.log('====================================================\n');

  console.log(`Target User: nakshatra1233@gmail.com`);
  console.log(`Device Registered Token: ${EXPO_PUSH_TOKEN}`);

  // 1. Force Stop App for 100% Killed State
  ...
  const screenRec = spawn('adb', ['-s', DEVICE_SERIAL, 'shell', 'screenrecord', '--time-limit=15', '/data/local/tmp/expo_live_proof.mp4']);
  ...
  console.log('Step 4: Dispatching Push via Expo Push API to registered ExponentPushToken...');
  ...
  console.log('\nStep 5: Capturing ADB Logcat trace for NotificationManager / Vibrator / SoundCraft...');
  try {
  ...
  try {
    execSync(`adb -s ${DEVICE_SERIAL} pull /data/local/tmp/expo_live_proof.mp4 expo_live_proof.mp4`);
    fs.copyFileSync('expo_live_proof.mp4', 'C:\\Users\\DELL\\.gemini\\antigravity\\brain\\af631f2d-472c-4b1a-a400-9353bd6e2483\\expo_live_proof.mp4');
    console.log('Successfully saved expo_live_proof.mp4 to artifacts directory!');
  } catch (e) {
    console.log('Video Pull Error:', e.message);
  }

  console.log('\n====================================================');
  console.log('   EXPO PUSH KILLED STATE TEST COMPLETE              ');
  console.log('====================================================');
}
