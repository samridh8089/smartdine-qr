/**
 * Forensic Reproduction: Floor Layout Status Sync Bug
 * Target: Verify state flow from Customer QR -> Order API -> Database -> Realtime -> Floor Layout
 */
...
const realtimeSrc = readFileSync(path.resolve(__dirname, '../src/lib/realtime.ts'), 'utf-8');
const floorLayoutSrc = readFileSync(path.resolve(__dirname, '../src/components/floorplan/FloorLayoutManager.tsx'), 'utf-8');
const tablesPageSrc = readFileSync(path.resolve(__dirname, '../src/app/(dashboard)/dashboard/tables/page.tsx'), 'utf-8');
...
console.log('  realtime.ts targetChannels includes:');
console.log('   ', targetChannels.replace(/\n\s*/g, ' ').trim());
console.log('  FloorLayoutManager.tsx listens on:');
...
const liveTableStates_table14: Record<string, any> = {
...
const liveTableStates_table5: Record<string, any> = {}; // Not even present in table_states yet
...
// 5. Trace Order API and DB Update
console.log('\nTEST 4: State Machine Transitions');
console.log('  Order status transitions:');
console.log('  - preparing -> served: Table must REMAIN occupied');
console.log('  - served -> completed: Table must become available');
console.log('  - preparing -> cancelled: Table must become available');
