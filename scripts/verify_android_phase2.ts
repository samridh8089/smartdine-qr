/**
 * CleverOps Phase 2: Android App Shell + Feature Parity Verification Suite
 * 
 * Verifies:
 * 1. App Shell Architecture (Splash, Navigation, Session, Deep links, Back handler)
 * 2. Feature & Permission Parity across all 6 roles:
 *    - Owner (Dashboard, Orders, Live Orders, Floor Layout, Tables, Inventory, Billing, Reports, Bookings, Staff, Settings, Audit Logs)
 *    - Manager (Restricted from Owner-only actions: Plan purchase, staff deletion)
 *    - Waiter (Assigned tables, Punch order with idempotency, Modify order, Guest count, Bookings, Table transfer)
 *    - KDS (5-stage progression: New -> Preparing -> Ready -> Served -> Completed + SLA timers)
 *    - Cashier (Bill calculation, Split bill with rupee conservation, GST computation, Payment settlement)
 *    - Customer (QR order flow & customer-facing Order ID concealing internal rest code)
 * 3. Historical Bug Regression Checks:
 *    - Inventory Bug (Deduction on preparing, restore on cancel, frozen engine diff = 0)
 *    - Staff OTP Bug (KeyboardAvoidingView safe, submit button visible, retry & loading states)
 *    - Punch Order Idempotency (Rapid taps synchronously locked via useRef, duplicate prevention)
 *    - Button Response latency (<100ms feedback simulation)
 * 4. Realtime Verification Chain:
 *    - Customer QR -> Waiter -> KDS -> Owner -> Cashier -> Back to Owner (Double-pass without refresh)
 * 5. Mobile UX Checklist (Keyboard safe, safe areas, responsive bottom sheets, modal closures)
 */
...
runTest('Invariant: packages/core/* remains strictly frozen (0 diff)', () => {
  const gitDiff = execSync('git diff packages/core', {
    encoding: 'utf8'
  }).trim();
...
runTest('Invariant: Inventory Engine & Units are strictly frozen (0 diff)', () => {
  const gitDiff = execSync('git diff src/lib/inventoryEngine.ts src/lib/inventoryUnits.ts', {
    encoding: 'utf8'
  }).trim();
...
runTest('Step 2: Waiter Role - Table Operations (Punch, Modify, Guest Count, Table Transfer)', () => {
  ...
  assert(waiterOrders.includes('Table Operations & Transfer'), 'WaiterOrdersScreen missing Table Operations modal');
...
runTest('Step 2: KDS Role - 5-Stage Lifecycle (New -> Preparing -> Ready -> Served -> Completed) & Live SLA', () => {
  ...
  assert(kitchenScreen.includes('getTimerColor') || kitchenScreen.includes('elapsed') || kitchenScreen.includes('Timer'), 'KitchenScreen missing SLA timer tracking');
});
...
runTest('Step 2: Cashier Role - Split Bill, Rupee Conservation, GST & Settlement', () => {
  ...
  assert(cashierScreen.includes('Split Bill & Settlement'), 'CashierScreen missing Split Bill modal');
  ...
  assert(cashierScreen.includes('Round-off'), 'CashierScreen missing integer rupee round-off display');
  assert(cashierScreen.includes('equalSplit'), 'CashierScreen missing equal split computation');
});
