...
async function measureNavigationSuite(viewport, isMobile = false) {
  console.log(`\n=== Running Post-Optimization Navigation Audit on ${viewport.width}x${viewport.height} (${isMobile ? 'Mobile' : 'Desktop'}) ===`);
  ...
  for (const target of ROUTES) {
    ...
    // Hover or touch to trigger prewarming like a real user
    if (isMobile) {
      await page.hover(linkSelector).catch(() => {});
    } else {
      await page.hover(linkSelector).catch(() => {});
    }
...
async function runAudit() {
  ...
  fs.writeFileSync('scratch/after_true_navigation.json', JSON.stringify(afterData, null, 2));
  console.log('\n=== Post-Optimization Navigation Metrics Saved to scratch/after_true_navigation.json ===');
