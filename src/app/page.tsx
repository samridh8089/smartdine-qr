    ...
    <div className="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 min-h-screen flex flex-col font-sans transition-colors duration-300">
      ...
      <nav className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 py-4 px-6 md:px-12 flex items-center justify-between shrink-0 sticky top-0 z-30 shadow-sm transition-colors">
        <Link href="/" className="flex items-center gap-3">
          ...
          <span className="font-extrabold text-base sm:text-lg tracking-tight text-slate-900 dark:text-white">CleverOps</span>
        </Link>
      ...
      {/* Hero Section */}
      <section className="px-6 md:px-12 pt-12 pb-16 md:pt-20 md:pb-24 max-w-5xl mx-auto text-center space-y-6">
        <div className="inline-flex items-center gap-2 bg-emerald-500/10 dark:bg-emerald-500/20 border border-emerald-500/20 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider text-emerald-700 dark:text-emerald-400">
          <Zap className="h-4 w-4" /> Next-Gen Restaurant Management SaaS
        </div>
        ...
        <p className="text-xs sm:text-sm md:text-lg text-slate-500 dark:text-slate-400 max-w-2xl mx-auto leading-relaxed">
          Increase table turnover, eliminate orders error, and reduce staff pressure. CleverOps is a multi-tenant ordering platform designed specifically for fast-casual restaurants, bars, and bistros.
        </p>
