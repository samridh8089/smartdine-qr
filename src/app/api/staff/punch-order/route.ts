  ...
  try {
    ...
    if (activeOrder) {
      ...
      const orderPayload = {
        ...
        special_instructions: [specialInstructions, finalCustomerName ? `Customer: ${finalCustomerName}${finalCustomerPhone ? ` (${finalCustomerPhone})` : ''}` : null].filter(Boolean).join(' | ') || null,
