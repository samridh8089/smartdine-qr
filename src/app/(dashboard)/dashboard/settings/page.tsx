          ...
          <form onSubmit={handleSaveNotificationSettings} className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-6">
                ...
                {/* Kitchen Bell Column */}
                <div className="p-4 bg-slate-50 dark:bg-slate-950/20 border border-slate-100 dark:border-slate-800 rounded-xl space-y-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="font-extrabold text-sm text-slate-900 dark:text-white block">Kitchen Portal Bell</span>
                      <span className="text-xs text-slate-400 mt-1 block">Continuous alert tone for new orders.</span>
                    </div>
                    <div className="flex gap-2">
                      <Button
                        type="button"
                        ...
                        onClick={() => {
                          const createBeepWavDataUri = () => {
                            const sampleRate = 8000; const duration = 0.5; const numSamples = sampleRate * duration;
                            ...
                            const beepSamples = sampleRate * 0.2; const frequency = 880;
                            for (let i = 0; i < numSamples; i++) {
                              buffer[44 + i] = (i < beepSamples) ? ((Math.sin(2 * Math.PI * frequency * (i / sampleRate)) >= 0) ? 200 : 56) : 128;
                            }
                          ...
                          window.dispatchEvent(new Event('stop-kitchen-sound')); // Stop global alarm if playing
                          (window as any)._previewAudio = new Audio(kitchenBellUrl || createBeepWavDataUri());
                          (window as any)._previewAudio.play().catch((e: any) => alert("Please interact with the page first."));
                        }}
                        className="rounded-xl px-4 py-2 font-bold cursor-pointer text-xs"
                      >
                        Preview Sound
                      </Button>
                      <Button
                        ...
                        onClick={() => {
                          ...
                          if ((window as any)._previewAudio) {
                            (window as any)._previewAudio.pause();
                            (window as any)._previewAudio = null;
                          }
                        }}
                        className="rounded-xl px-4 py-2 font-bold cursor-pointer text-xs bg-rose-500 text-white"
                      >
                        Stop Sound
                      </Button>
                    </div>
                  ...
                  <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Upload Custom Kitchen Bell (Optional)</label>
                    <div className="flex items-center gap-3">
                      <input
                        type="file"
                        accept="audio/mp3, audio/wav, audio/mpeg, audio/x-wav, audio/m4a, audio/x-m4a"
                        onChange={async (e) => {
                          const file = e.target.files?.[0];
                          if (!file) return;
                          ...
                          try {
                            if (kitchenBellUrl) {
                              await storage.deleteImage(kitchenBellUrl);
                            }
                            const url = await storage.uploadAudio(file, restaurant?.id || '', 'kitchen-bell');
                            setKitchenBellUrl(url);
                            setKitchenBellType('custom'); // Ensure it registers as custom if needed for legacy
                            alert("Kitchen bell file uploaded successfully! Click save configuration to apply changes.");
                          } catch (err: any) {
                        ...
                        className="text-xs block w-full border border-slate-200 dark:border-slate-800 rounded-xl p-2 bg-white dark:bg-slate-900 file:mr-4 file:py-1 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-extrabold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 cursor-pointer"
                      />
                      {uploadingKitchen && <span className="text-xs text-slate-400 animate-pulse">Uploading...</span>}
                    </div>
                    {kitchenBellUrl ? (
                      <div className="flex items-center justify-between mt-2">
                        <p className="text-[10px] text-emerald-600 font-mono font-bold truncate">Active custom sound: {kitchenBellUrl}</p>
                        <button type="button" onClick={() => setKitchenBellUrl('')} className="text-xs text-rose-500 font-bold hover:underline">Remove</button>
                      </div>
                    ) : (
                      <p className="text-[10px] text-slate-400 mt-2">Using default notification sound.</p>
                    )}
                  </div>
                </div>
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-950/20 border border-slate-100 dark:border-slate-800 rounded-xl space-y-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="font-extrabold text-sm text-slate-900 dark:text-white block">Waiter Portal Bell</span>
                      <span className="text-xs text-slate-400 mt-1 block">Continuous alert tone for calls & ready food.</span>
                    </div>
                    <div className="flex gap-2">
                      <Button
                        type="button"
                        ...
                        onClick={() => {
                          ...
                          window.dispatchEvent(new Event('stop-waiter-sound')); // Stop global alarm if playing
                          (window as any)._previewAudioWaiter = new Audio(waiterBellUrl || createBeepWavDataUri());
                          (window as any)._previewAudioWaiter.play().catch((e: any) => alert("Please interact with the page first."));
                        }}
                        className="rounded-xl px-4 py-2 font-bold cursor-pointer text-xs"
                      >
                        Preview Sound
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          if ((window as any)._previewAudioWaiter) {
                            (window as any)._previewAudioWaiter.pause();
                            (window as any)._previewAudioWaiter = null;
                          }
                        }}
                        className="rounded-xl px-4 py-2 font-bold cursor-pointer text-xs bg-rose-500 text-white"
                      >
                  ...
                  <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Upload Custom Waiter Bell (Optional)</label>
                    <div className="flex items-center gap-3">
                      <input
                        type="file"
                        accept="audio/mp3, audio/wav, audio/mpeg, audio/x-wav, audio/m4a, audio/x-m4a"
                        onChange={async (e) => {
                          const file = e.target.files?.[0];
                          if (!file) return;
                          setUploadingWaiter(true);
                          try {
                            if (waiterBellUrl) {
                              await storage.deleteImage(waiterBellUrl);
                            }
                            const url = await storage.uploadAudio(file, restaurant?.id || '', 'waiter-bell');
                            setWaiterBellUrl(url);
                            setWaiterBellType('custom'); // Ensure it registers as custom if needed for legacy
                            alert("Waiter bell file uploaded successfully! Click save configuration to apply changes.");
                          } catch (err: any) {
                            alert(err.message || "Upload failed");
                          } finally {
                            setUploadingWaiter(false);
                          }
                        }}
                        disabled={uploadingWaiter}
                        className="text-xs block w-full border border-slate-200 dark:border-slate-800 rounded-xl p-2 bg-white dark:bg-slate-900 file:mr-4 file:py-1 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-extrabold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 cursor-pointer"
                      />
                      {uploadingWaiter && <span className="text-xs text-slate-400 animate-pulse">Uploading...</span>}
                    </div>
                    {waiterBellUrl ? (
                      <div className="flex items-center justify-between mt-2">
                        <p className="text-[10px] text-emerald-600 font-mono font-bold truncate">Active custom sound: {waiterBellUrl}</p>
                        <button type="button" onClick={() => setWaiterBellUrl('')} className="text-xs text-rose-500 font-bold hover:underline">Remove</button>
                      </div>
                    ) : (
                      <p className="text-[10px] text-slate-400 mt-2">Using default notification sound.</p>
                    )}
